...
def load_all_data():
    ...
    st.error("No se encontro ningun CSV de Rancho Flamboyant.")
    return None
...
if df is not None:
    ...
    def horizontal_bar_chart(df_plot, x_col, y_col, color_scale, title, height=500):
        ...
        fig = go.Figure(go.Bar(x=df_sorted[x_col], y=df_sorted[y_col], orientation='h', text=[f"$ {v:,.2f}" for v in df_sorted[x_col]], textposition='outside'))
    ...
    with t4:
        ...
        if q:
            ...
            st.dataframe(res[cols_show].style.format({c: "${:,.2f}" for c in ['HONORARIOS', 'COSTO TOTAL']}), use_container_width=True)
